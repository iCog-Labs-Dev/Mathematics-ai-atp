...
class _Graph:
    def __init__(self, solved: bool, cooperative_deadline: bool = False):
        self._solved = solved
        self.cooperative_deadline = cooperative_deadline
...
class _Result:
    def __init__(self, theorem: str):
        ...
        self.graph = _Graph(
            theorem.startswith("solve"),
            cooperative_deadline=theorem == "cooperative-deadline",
        )
...
async def _create_pool(
    *,
    ...
    pool = await LiveReasonerPool.create(
        ...
        is_cooperative_deadline=lambda result: result.graph.cooperative_deadline,
...
def test_hard_timeout_replaces_worker_before_next_theorem():
    async def run():
        ...
        pool = await LiveReasonerPool.create(
            ...
            search_factory=search_factory,
            is_solved=lambda result: result.graph.is_solved(),
            is_cooperative_deadline=lambda result: False,
            startup_timeout_s=1.0,
            shutdown_timeout_s=1.0,
            log=lambda message: None,
        )
    ...
    assert result.stats["searches_hard_timed_out"] == 1.0
    assert result.stats["workers_replaced"] == 1.0


def test_cooperative_deadline_keeps_partial_result_without_replacing_worker():
    async def run():
        pool, _model, servers = await _create_pool(workers=1)
        async with pool:
            result = await pool.collect_round(
                ["cooperative-deadline", "solve-after-deadline"],
                timeout_s=1.0,
                model_generation=0,
            )
        return result, servers

    result, servers = asyncio.run(run())
    assert [item.theorem for item in result.results] == [
        "cooperative-deadline",
        "solve-after-deadline",
    ]
    assert result.records[0].outcome == "cooperative_deadline"
    assert result.stats["searches_cooperative_deadline"] == 1.0
    assert result.stats["searches_hard_timed_out"] == 0.0
    assert result.stats["workers_replaced"] == 0.0
...
def test_replica_sync_copies_final_state_and_advances_generation():
    ...
    with torch.no_grad():
        # Represent one on-policy mutation followed by two HTPS mutations.
        canonical.weight.add_(1.0)
        canonical.bias.sub_(1.0)
        canonical.weight.add_(1.0)
        canonical.bias.sub_(0.5)
        canonical.weight.add_(3.0)
        canonical.bias.sub_(0.5)

    pool.synchronize(canonical, 3)
    pool.assert_generation(3)

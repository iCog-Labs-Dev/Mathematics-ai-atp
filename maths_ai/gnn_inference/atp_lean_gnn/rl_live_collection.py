        ...
        *,
            ...
            while True:
                ...
                try:
                    result = await asyncio.wait_for(
                        self.search_factory(worker.reasoner, theorem, greedy),
                        timeout=timeout_s,
                    )
                    outcome: CollectionOutcome = (
                        "solved" if self.is_solved(result) else "unsolved"
                    )
                    records.append(
                        CollectionRecord(
                            batch_index=index,
        ...
        stats: dict[str, float] = {
            ...
            "searches_failed": float(
                sum(record.outcome in {"timeout", "error"} for record in records)
            ),
            "searches_timed_out": float(
                sum(record.outcome == "timeout" for record in records)
            ),
        ...
        for worker in workers:
            ...
            stats[f"worker_{worker.worker_id}_failed"] = float(
                sum(record.outcome in {"timeout", "error"} for record in worker_records)
            )

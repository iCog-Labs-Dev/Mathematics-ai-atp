...
device selection. Each effective worker always owns exactly one Pantograph server;
there is no separately adjustable server count. The canonical model and optimizer
remain on the configured update device. After an optimizer step, the updated
parameters are copied to every collection replica before the next inference boundary.
...
For the target 12-core, two-RTX-3090 host, start with this explicit profile:
...
The driver logs effective CPU capacity, worker/server count, active GPU memory, and
canonical model size before collection. Per-round metrics include queue wait, search
time, hard timeouts, worker replacements, collection wall-clock time, optimizer time,
replica generation, synchronization time, and per-worker attempt/failure counts.
Increase the worker count only after checking Lean process memory, theorem timeout
rate, and collection time.

A hard theorem timeout cancels an in-flight Pantograph response read. The driver
therefore closes and reconstructs that complete worker before assigning another
theorem to it. Other workers continue processing their queues. Ordinary theorem
errors remain isolated to their individual collection records.
